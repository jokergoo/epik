...
source(qq("@{BASE_DIR}/scripts/lib/utils.R"))
...
##################################################
## update annotation data frame if there is subgroup classification
if(file.exists(qq("@{OUTPUT_DIR}/rds/group_classification.rds"))) {
    res = readRDS(qq("@{OUTPUT_DIR}/rds/group_classification.rds"))
    col = readRDS(qq("@{OUTPUT_DIR}/rds/group_classification_color.rds"))
    SAMPLE = cbind(SAMPLE, subgroup = paste0("subgroup", res[sample_id, "km_expr"]))
    COLOR$subgroup = col$km_expr
    names(COLOR$subgroup) = paste0("subgroup", names(col$km_expr))
}
