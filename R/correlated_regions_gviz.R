...
#
cr_gviz = function(sig_cr, gi, expr, txdb, gene_start = NULL, gene_end = NULL, 
	species = "hg19", gf_list = NULL, hm_list = NULL, title = gi) {
	...
	if(is.null(gene_start) || is.null(gene_end)) {
		gene = genes(txdb)
		if(is.null(gene_start)) gene_start = start(gene[gi])
		if(is.null(gene_end)) gene_end = end(gene[gi])
	}
